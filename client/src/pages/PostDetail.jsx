...
import { FaCalendarAlt, FaComments, FaCubes, FaHeart, FaPen, FaTrash, FaUser } from "react-icons/fa";
...
const PostDetail = () => {
  ...
  const { data: comments } = useSWR(`${URL}/comments/${post_id}?limit=3`, fetcher);
  ...
  const handleLikeUnlikePost = async () => {
    if (window.confirm('Like/Unlike this post?')) {
      try {
        const likeDetail = { user_id, post_id };
        const response = await fetch(`${URL}/posts/like-and-unike-post`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${user.token}`
  ...
  const handleComment = async (e) => {
    e.preventDefault();
    if (window.confirm('Add this comment?')) {
      try {
      ...
      {post_isLoading ? (
        ...
        <section className="container mx-auto max-w-5xl flex flex-col px-5 py-5 lg:flex-row lg:gap-x-5 lg:items-start mt-8">
          <article className="flex-1">
            <div className="img-div">
              <div className="flex justify-end items-center gap-2 mb-1">
                ...
                <form onSubmit={handleRemovePost}>
                  <button type='submit' className='cate-delete-btn'>
                    <FaTrash />
                  </button>
                </form>
              </div>
              <img
                ...
                src={post?.data?.photo || samplePostImage}
                alt={post?.data?.title}
              />
            </div>
            ...
            <div className="flex justify-between items-center my-3">
              ...
              <div className="flex justify-end items-center gap-3.5 text-dark-light">
                <div className="flex items-center gap-1 text-sm cursor-pointer" onClick={handleLikeUnlikePost}>
                  <FaHeart className={`${post?.data?.likes?.includes(user_id) ? 'text-red-600' : 'text-gray-400'}`} />
            ...
            <h1 className="text-xl font-medium font-roboto mt-4 text-dark-hard md:text-[26px] flex justify-between items-center">
              {post?.data?.title}
              <div className="flex items-center gap-1.5 text-dark-light">
                <FaCubes size={17} />
                <Link to={`/blog?category=${post?.data?.categories?.name}`} className="text-sm md:text-base">
